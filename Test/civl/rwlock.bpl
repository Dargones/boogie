...
function {:constructor} RwLock(writer: OptionTid, readers: [Tid]bool): RwLock;
...
{
    assume writer#RwLock(rwlock) == None();
    assume (forall tid: Tid :: !readers#RwLock(rwlock)[tid]);
...
function {:inline} holds_write_lock(tid: Tid, rwlock: RwLock): bool
{
    ...
    (forall tid: Tid :: !readers#RwLock(rwlock)[tid])
}
